...
KN_WLAZ80FLAGS ?= -I $(KN_DIR) -I $(KN_TYPE_DIR) -D KN_SONG_SLOTS=$(KN_SONG_SLOTS)
KN_WLALINKFLAGS ?= -b
...
KN_OUT_MODULE_CONSTANTS ?= $(KN_TYPE_DIR)KN-MODULE-CONSTANTS.asm
...
kn-clean: kn-type-clean
	-$(RM) $(KN_OUT_CONVERT) $(KN_OUT_GENERATE_DATA) \
		 $(KN_OUT_COMPILED_MODULE) $(KN_OUT_MODULE_CONSTANTS) $(KN_OUT_GENERATED_DATA)
...
$(KN_OUT_COMPILED_MODULE) $(KN_OUT_MODULE_CONSTANTS): $(KN_OUT_CONVERT) $(KN_MODULES)
	$(KN_OUT_CONVERT) $(KN_TYPE) $(KN_OUT_COMPILED_MODULE) $(KN_OUT_MODULE_CONSTANTS) $(KN_MODULES)
